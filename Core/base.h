...
int CodeParser::parse_args(string& code){
    ...
    *write_to += "(";
    ...
    while(code.substr(0,1)!=")" and function_code_prev!=code){
        ...
        //Get Value Type
        if(code_parser.harvest_value_type(code,argument_type)==EXIT_FAILURE){
        ...
        //Handle Value
        if(code_parser.harvest_value(code,argument_type,",)")==EXIT_FAILURE){
            return EXIT_FAILURE;
        }
...
int CodeParser::harvest_value(string& code, string &type, string additional_characters, string method){
    /*
        code - code
        type - variable type
...
int CodeParser::harvest_value_type(string code, string &type, string method){
    /*
        code - code
